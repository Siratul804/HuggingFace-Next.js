...
export default function HugImg() {
  const [messages, setMessages] = useState([
    { id: 1, text: "Hello! Send me image", sender: "bot" },
  ]);
  ...
  const handleSendMessage = async () => {
    if (inputMessage.trim() !== "") {
      ...
      try {
        const response = await fetch("/api/hugImg", {
          method: "POST",
          headers: {
    ...
    <>
      <div className="flex items-center justify-center flex-col h-screen bg-white">
        <div className="text-center py-4 ">
          ...
          <p className="text-sm">
            meta-llama/Llama-3.2-11B-Vision-Instruct model developed by the{" "}
            <br />
            Meta Team, focused on advancing AI reasoning capabilities where img
            to text takes place.
          </p>
        </div>
